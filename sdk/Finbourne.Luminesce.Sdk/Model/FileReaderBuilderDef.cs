...
namespace Finbourne.Luminesce.Sdk.Model
{
    ...
    public partial class FileReaderBuilderDef : IEquatable<FileReaderBuilderDef>, IValidatableObject
    {
        ...
        {
            // VariableName (string) maxLength
            if (this.VariableName != null && this.VariableName.Length > 256)
            {
                yield return new System.ComponentModel.DataAnnotations.ValidationResult("Invalid value for VariableName, length must be less than 256.", new [] { "VariableName" });
            }

            // VariableName (string) minLength
            if (this.VariableName != null && this.VariableName.Length < 0)
            {
                yield return new System.ComponentModel.DataAnnotations.ValidationResult("Invalid value for VariableName, length must be greater than 0.", new [] { "VariableName" });
            }

            // FilePath (string) maxLength
            if (this.FilePath != null && this.FilePath.Length > 256)
            {
                yield return new System.ComponentModel.DataAnnotations.ValidationResult("Invalid value for FilePath, length must be less than 256.", new [] { "FilePath" });
            }

            // FilePath (string) minLength
            if (this.FilePath != null && this.FilePath.Length < 0)
            {
                yield return new System.ComponentModel.DataAnnotations.ValidationResult("Invalid value for FilePath, length must be greater than 0.", new [] { "FilePath" });
            }

            // FolderFilter (string) maxLength
            if (this.FolderFilter != null && this.FolderFilter.Length > 256)
            {
                yield return new System.ComponentModel.DataAnnotations.ValidationResult("Invalid value for FolderFilter, length must be less than 256.", new [] { "FolderFilter" });
            }

            // FolderFilter (string) minLength
            if (this.FolderFilter != null && this.FolderFilter.Length < 0)
            {
                yield return new System.ComponentModel.DataAnnotations.ValidationResult("Invalid value for FolderFilter, length must be greater than 0.", new [] { "FolderFilter" });
            }

            // ZipFilter (string) maxLength
            if (this.ZipFilter != null && this.ZipFilter.Length > 256)
            {
                yield return new System.ComponentModel.DataAnnotations.ValidationResult("Invalid value for ZipFilter, length must be less than 256.", new [] { "ZipFilter" });
            }

            // ZipFilter (string) minLength
            if (this.ZipFilter != null && this.ZipFilter.Length < 0)
            {
                yield return new System.ComponentModel.DataAnnotations.ValidationResult("Invalid value for ZipFilter, length must be greater than 0.", new [] { "ZipFilter" });
            }
